"""
Verify ToFu Refactor (Lightweight)
"""
import os
import requests
import time
...
TEST_DOMAIN = "test-tofu-refactor.com"

def run_test():
    print("🚀 Starting ToFu Refactor Verification...")
    
    # 1. Setup Project
    print("\n1. Setting up Project...")
    proj_res = supabase.table('projects').insert({
        "domain": TEST_DOMAIN,
        "project_name": "ToFu Refactor Test"
    }).execute()
    ...
    mofu_url = "https://www.rhodeskin.com/pages/refactor-test"
    ...
    try:
        ...
        }, timeout=60) # 60s timeout (Grounding takes time)
        ...
        if tofu_pages.data:
            ...
            for p in tofu_pages.data:
                print(f"     - {p['tech_audit_data'].get('title')} (Action: {p.get('product_action')})")
                
                # Verify Research Data is minimal
                r_data = p.get('research_data') or {}
                if 'perplexity_research' not in r_data:
                    print("       ✓ No Perplexity Research (Correct)")
                else:
                    print("       ❌ Perplexity Research Found (Incorrect)")
        else:
